...
/*
 * This approach programmatically sets up the text and presents a UI
 * where the user can use Writing Tools to rewrite in the desired tone.
 
 @available(iOS 18.0, *)
 struct ContentView: View {
     @StateObject private var rewriter = BirthdayMessageRewriter()
     @State private var finalMessage: String = ""
     
     var body: some View {
         VStack(spacing: 20) {
             Button("Rewrite Birthday Message") {
                 rewriter.rewriteMessage(
                     originalMessage: "Happy birthday! Hope you have a great day!",
                     tone: "funny"
                 ) { rewrittenMessage in
                     finalMessage = rewrittenMessage
         ...
         .sheet(isPresented: $rewriter.showRewriteSheet, onDismiss: {
             rewriter.handleDismiss()
         }) {
             VStack {
                 rewriter.makeRewriteView()
                 
                 Button("Done") {
                     rewriter.showRewriteSheet = false
                 }
                 .buttonStyle(.borderedProminent)
                 .padding()
             }
 ...
 * When you call rewriteMessage(), it will:
 * 1. Set the original message in a text editor
 * 2. Present a sheet with instructions for the specific tone
 * 3. Allow the user to use Writing Tools to rewrite
 * 4. Return the rewritten message via the completion handler
 */
